#!/usr/bin/env bash
# 绘制 Ours 相对各 baseline 的 QoE 改进 CDF（需已跑完测试）
set -euo pipefail
...
"${PYTHON}" run/plot_qoe_improvement_cdf.py \
  ...
  --test-rounds "${ROUNDS}" \
  --ours-dir "${OURS_DIR}" \
  --output "artifacts/figures/qoe_improvement_cdf_${TRACE}_${VIDEO}.png" \
  --stats-json "artifacts/figures/qoe_improvement_cdf_${TRACE}_${VIDEO}.json" \
  "$@"
