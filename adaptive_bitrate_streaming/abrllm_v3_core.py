"""
ABRLLM v3 核心网络（自 v2 复制，不 import ABRLLM_v2）。
含 StateEncoder / AlignmentLayer 与跨模态对比对齐损失接口。
"""
import random
...
class ABRLLM(nn.Module):
    def __init__(self, args):
        ...
        self.data_description = (
            ...
            "2) Network State (including last bitrate, buffer level, throughput history, "
            "download time history, upcoming chunk sizes, and remaining chunks), "
        ...
        # 跨模态对比：alignment 后的 state_embed ↔ 冻结 PLM 编码的 joint 文本
        self.align_temperature = float(getattr(args, 'align_temperature', 0.07))
        contrast_dim = int(getattr(args, 'contrast_dim', 0) or 0)
        self.use_contrast_proj = contrast_dim > 0
        out_dim = contrast_dim if self.use_contrast_proj else self.llm_dim
        if self.use_contrast_proj:
            contrast_hidden = int(getattr(args, 'contrast_hidden_dim', 0) or 0)
            if contrast_hidden <= 0:
                contrast_hidden = min(self.llm_dim, max(out_dim * 2, 256))
            self.contrast_hidden_dim = contrast_hidden
            self.contrast_proj_state = self._build_contrast_projector(
                self.llm_dim, contrast_hidden, out_dim
            ).to(self.device)
            self.contrast_proj_text = self._build_contrast_projector(
                self.llm_dim, contrast_hidden, out_dim
            ).to(self.device)
        else:
            self.contrast_hidden_dim = self.llm_dim
            self.contrast_proj_state = nn.Identity()
            self.contrast_proj_text = nn.Identity()
        self.contrast_dim = out_dim
        ...
        if self.use_contrast_proj:
            modules_list.extend([self.contrast_proj_state, self.contrast_proj_text])
    ...
    @staticmethod
    def _build_contrast_projector(in_dim: int, hidden_dim: int, out_dim: int) -> nn.Sequential:
        """llm_dim → hidden → contrast_dim，非线性保留更丰富的 state/文本细节。"""
    ...
    def forward(self, states, actions, returns, timesteps, attention_mask=None, return_state_aligned=False):
        """
        ...
        Args:
            ...
            return_state_aligned: 若 True，额外返回 alignment_layer 输出（未加 timestep）
        
        Returns:
            action_pred 或 (action_pred, state_aligned)
        """
        ...
        state_aligned = state_embeddings
        ...
        if attention_mask is None:
            attention_mask = torch.ones((concated_embeddings.shape[0], concated_embeddings.shape[1]), 
        ...
        if isinstance(transformer_outputs, dict):
            output = transformer_outputs['hidden_states'][-1]
        else:
            output = transformer_outputs.hidden_states[-1] if hasattr(transformer_outputs, 'hidden_states') else transformer_outputs.last_hidden_state
        ...
        if return_state_aligned:
            return action_pred, state_aligned
        return action_pred

    @torch.no_grad()
    def encode_texts(self, texts, max_length=256, batch_size=32):
        """冻结 PLM：对文本列表做 token 均值池化，得到 (N, llm_dim)。"""
        if not texts:
            return torch.zeros((0, self.llm_dim), device=self.device, dtype=torch.float32)
        embed_layer = self.plm.get_input_embeddings()
        out = []
        for start in range(0, len(texts), batch_size):
            ...
            tok_emb = embed_layer(input_ids).float()
            pooled = (tok_emb * attn).sum(dim=1) / attn.sum(dim=1).clamp(min=1e-6)
            out.append(pooled)
        return torch.cat(out, dim=0)

    def _project_contrast_state(self, state_aligned):
        return self.contrast_proj_state(state_aligned)

    def _project_contrast_text(self, text_emb):
        return self.contrast_proj_text(text_emb)

    def compute_align_loss(self, state_aligned, states, labels):
        """
        InfoNCE：alignment 后的 state_embed vs 专家 joint 文本（正）/ 错动作 joint 文本（负）。

        :param state_aligned: (B, T, llm_dim)，alignment_layer 输出（未加 timestep）
        :param states: (B, T, 6, 6)
        :param labels: (B, T) 硬标签码率档位
        """
        from plm_special.utils.losses import info_nce_contrastive_loss

        from ABRLLM_v3 import render_joint_state_action_text

        batch_size, seq_len = state_aligned.shape[:2]
        ...
        for b in range(batch_size):
            for t in range(seq_len):
                s = states_np[b, t]
                a = int(labels_np[b, t])
                prev = int(labels_np[b, t - 1]) if t > 0 else None
                pos_texts.append(
                    render_joint_state_action_text(s, a, prev_action=prev)
                )
                for na in range(BITRATE_LEVELS):
                    if na == a:
                        continue
                    neg_texts.append(
                        render_joint_state_action_text(s, na, prev_action=prev)
                    )

        text_pos = self.encode_texts(pos_texts)
        text_neg = self.encode_texts(neg_texts)
        n_neg = BITRATE_LEVELS - 1
        text_neg = text_neg.view(batch_size * seq_len, n_neg, self.llm_dim)

        z_state = self._project_contrast_state(state_aligned.reshape(batch_size * seq_len, -1))
        z_pos = self._project_contrast_text(text_pos)
        z_neg = self._project_contrast_text(text_neg.reshape(-1, self.llm_dim)).view(
            batch_size * seq_len, n_neg, -1
        )
        return info_nce_contrastive_loss(
            z_state, z_pos, z_neg, temperature=self.align_temperature
        )
    ...
    def sample(self, state, target_return, timestep, **kwargs):
        ...
        # Safely extract hidden states (same as forward method)
        if isinstance(transformer_outputs, dict):
            output = transformer_outputs['hidden_states'][-1]
        else:
            output = transformer_outputs.hidden_states[-1] if hasattr(transformer_outputs, 'hidden_states') else transformer_outputs.last_hidden_state
