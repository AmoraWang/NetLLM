...
# When generating oracle experience pools, optionally dump per-chunk logs
# (for analysis/debug) under this directory.
DP_RESULTS_DIR = os.path.join(_ABR_ROOT, 'artifacts', 'results', 'DP')
...
def generate_oracle_exp_pool(
    ...
    seed: int = 100003,
    log_dir=None,
    log_file_prefix: str = 'result_sim_abr_DP',
):
    """
    ...
    # Prepare log directory (optional).
    ...
    for trace_idx, trace_path in enumerate(trace_files):
        ...
        # DP replay for logging: cumulative "time_stamp" (ms) is approximated as
        # sum of download delays (DP does not include buffer drain sleep-time).
        time_stamp_ms = 0.0
        
        # 记录处理前经验池的长度，用于调试
        transitions_before = len(exp_pool)

        # Open per-trace log file if requested.
        log_file = None
        if log_dir is not None:
            trace_name = os.path.basename(trace_path)
            log_path = os.path.join(log_dir, f'{log_file_prefix}_{trace_name}')
        ...
        for chunk_idx, action_idx in enumerate(actions):
            ...
            if chunk_idx == 0:
                ...
                smoothness_diff = abs(curr_bitrate_kbps - VIDEO_BIT_RATE[last_bit_rate_idx]) / M_IN_K

            r_smooth = SMOOTH_PENALTY * smoothness_diff
            reward = r_quality - r_rebuf - r_smooth
            step_rewards.append(reward)

            # DP replay "time_stamp" 近似为累计下载时延（DP 不包含 buffer drain sleep_time）。
            ...
            state[5, -1] = min(remain_chunks, CHUNK_TIL_VIDEO_END_CAP) / float(CHUNK_TIL_VIDEO_END_CAP)
            ...
            # Write per-chunk DP replay log (8 columns) if enabled.
            if log_file is not None:
                # time_stamp bit_rate buffer_size rebuffer_time chunk_size download_time smoothness reward
                log_file.write(
                    f'{time_stamp_ms / M_IN_K}\t'
                    f'{curr_bitrate_kbps}\t'
                    f'{buffer_size_sec}\t'
                    f'{rebuf_sec}\t'
                    f'{video_chunk_size}\t'
...
def run_oracle_dp_test(
    ...
    *,
    ...
    save_exp_pool: bool = True,
    log_file_prefix: str = "result_sim_abr",
) -> dict:
    """
    在测试集 trace 上运行 DP Oracle，写出与 ``plm_special/test`` 一致的仿真日志，
    供 ``plot_qoe_cdf.py`` / ``calc_mean_reward`` 使用。

    输出目录（与 rule_baseline 等对齐）::
        artifacts/results/{trace}_{video}/trace_num_{N}_fixed_{True|False}/DP/seed_{seed}_rounds_{R}/

    Returns:
        含 results_dir、mean_qoe、num_traces 等字段的字典。
    """
    from config import cfg
    from plm_special.utils.utils import calc_mean_reward

    if trace not in cfg.trace_dirs:
        raise KeyError(f"未知 trace: {trace!r}，请在 config.cfg.trace_dirs 中配置")
    if video not in cfg.video_size_dirs:
        raise KeyError(f"未知 video: {video!r}，请在 config.cfg.video_size_dirs 中配置")

    trace_folder = cfg.trace_dirs[trace]
    video_size_dir = cfg.video_size_dirs[video]
    if not os.path.isdir(trace_folder):
        raise FileNotFoundError(f"trace 目录不存在: {trace_folder}")
    if not os.path.isdir(video_size_dir):
        raise FileNotFoundError(f"video 目录不存在: {video_size_dir}")

    trace_files = sorted(glob.glob(os.path.join(trace_folder, "*")))
    n_available = len(trace_files)
    limit = n_available if trace_num in (-1, None) else min(trace_num, n_available)
    if limit <= 0:
        raise ValueError(f"trace 目录 {trace_folder!r} 下无可用文件")

    rounds_tag = f"seed_{seed}_rounds_{test_rounds}"
    results_dir = os.path.join(
        cfg.results_dir,
        f"{trace}_{video}",
        f"trace_num_{limit}_fixed_{fixed_order}",
        "DP",
        rounds_tag,
    )
    os.makedirs(results_dir, exist_ok=True)

    exp_pool_path = os.path.join(results_dir, "oracle_exp_pool.pkl") if save_exp_pool else None
    dummy_pkl = os.path.join(results_dir, "_oracle_no_save.pkl")

    print(f"\n=== DP Oracle 测试轨迹 ===")
    print(f"  trace={trace} ({trace_folder})")
    print(f"  video={video}")
    print(f"  trace_num={limit} / available={n_available}")
    print(f"  results_dir={results_dir}")

    generate_oracle_exp_pool(
        trace_folder=trace_folder,
        video_size_dir=video_size_dir,
        output_path=exp_pool_path or dummy_pkl,
        trace_limit=limit,
        seed=seed,
        log_dir=results_dir,
    ...
    if not save_exp_pool and os.path.isfile(dummy_pkl):
        os.remove(dummy_pkl)

    log_files = [
        f for f in os.listdir(results_dir)
        if log_file_prefix in f and os.path.isfile(os.path.join(results_dir, f))
    ]
    mean_qoe = calc_mean_reward(log_files, results_dir, str="", skip_first_reward=True, skip_last_reward=True)
    ...
    summary = {
        ...
        "trace_num": limit,
        "seed": seed,
        "mean_qoe": float(mean_qoe),
        "num_log_files": len(log_files),
        "exp_pool_path": exp_pool_path,
    }
    print(f"  mean_qoe (skip 1st/last chunk, 与 baseline 一致): {mean_qoe:.6f}")
    print(f"  日志条数: {len(log_files)}")
    return summary
...
def _build_cli_parser():
    ...
    parser = argparse.ArgumentParser(
        description="DP Oracle：生成最优 ABR 轨迹（经验池和/或测试仿真日志）",
    )
    parser.add_argument(
        "--mode",
        choices=("test", "exp-pool"),
        default="test",
        help="test=测试集仿真日志（默认）；exp-pool=仅生成经验池 pickle",
    )
    parser.add_argument("--trace", default="fcc-test", help="config.cfg.trace_dirs 中的键")
    parser.add_argument("--video", default="video1", help="config.cfg.video_size_dirs 中的键")
    parser.add_argument(
        "--trace-num",
        type=int,
        default=100,
        help="使用的 trace 条数；-1 表示目录内全部",
    )
    parser.add_argument("--seed", type=int, default=666)
    parser.add_argument("--test-rounds", type=int, default=1, help="仅用于结果目录命名")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="--mode exp-pool 时经验池 pickle 路径",
    )
    parser.add_argument(
        "--no-save-exp-pool",
        action="store_true",
        help="--mode test 时不保留 oracle_exp_pool.pkl",
    )
    parser.add_argument(
        "--log-dir",
        default=None,
        help="--mode exp-pool 时可选日志目录",
    )
    return parser
...
def main_cli(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "test":
        run_oracle_dp_test(
            ...
            save_exp_pool=not args.no_save_exp_pool,
        )
        return 0

    from config import cfg

    trace_folder = cfg.trace_dirs.get(args.trace)
    video_size_dir = cfg.video_size_dirs.get(args.video)
    if trace_folder is None or video_size_dir is None:
        raise SystemExit(f"未知 trace/video: {args.trace} / {args.video}")

    output_path = args.output or os.path.join(
        cfg.exp_pools_dir,
        f"oracle_{args.trace}_{args.video}.pkl",
    )
