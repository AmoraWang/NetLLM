...
def adapt(args, model, exp_dataset, exp_dataset_info, eval_env_settings, checkpoint_dir, best_model_dir, eval_process_reward_fn):
    ...
    print(f'Training loss: {args.loss_type}')
    if args.loss_type == 'ce_kl':
        print(
            f'  CE+KL: kd_alpha={args.kd_alpha}, kd_temperature={args.kd_temperature}, '
            f'teacher_is_prob={args.teacher_is_prob}'
        )
...
def test(args, model, exp_dataset_info, env_settings, model_dir, result_dir, test_process_reward_fn):
    ...
    results = test_on_env(args, model, result_dir, env_settings, target_return, args.trace_num, test_process_reward_fn, seed=args.seed,
                          num_test_rounds=args.test_rounds)
    print(results)
...
def run(args):
    ...
    if args.loss_type == 'ce_kl':
        if not getattr(exp_pool, 'has_teacher_logits', False):
            raise ValueError(
                "loss_type='ce_kl' requires experience pool field 'teacher_logits' "
                f"(length must equal len(states)). Got pool without teacher logits: {args.exp_pool_path}"
            )
    if getattr(args, 'abr_llm_version', 'v2') == 'v3':
        from ABRLLM_v3 import ABRLLM as ABRLLMCls
    ...
    models_dir = os.path.join(
        ...
        f'abrllm_{getattr(args, "abr_llm_version", "v2")}_rank_{args.rank}_w_{args.w}_gamma_{args.gamma}_sfd_{args.state_feature_dim}'
    ...
    results_dir = os.path.join(
        ...
        f'{args.plm_type}_{args.plm_size}',
        f'abrllm_{getattr(args, "abr_llm_version", "v2")}_rank_{args.rank}_w_{args.w}_gamma_{args.gamma}_tgt_scale_{args.target_return_scale}_seed_{args.seed}'
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=('ce', 'ce_kl'),
        default='ce',
        help="ce: hard-label CrossEntropy only; ce_kl: alpha*CE + (1-alpha)*KL(teacher||student), requires teacher_logits in exp pool",
    )
    parser.add_argument(
        '--kd-alpha',
        type=float,
        default=0.5,
        help='Weight on hard CE when loss_type=ce_kl; total = kd_alpha*CE + (1-kd_alpha)*KL',
    )
    parser.add_argument(
        '--kd-temperature',
        type=float,
        default=2.0,
        help='Distillation temperature when loss_type=ce_kl',
    )
    parser.add_argument(
        '--teacher-is-prob',
        action='store_true',
    ...
    parser.add_argument('--test-rounds', type=int, default=1,
                        help='测试轮数；>1 时每轮 time_ns 随机带宽起点，mean_qoe 为多轮平均')
    ...
    if args.loss_type == 'ce_kl':
        assert 0.0 <= args.kd_alpha <= 1.0, 'kd_alpha must be in [0, 1]'
        assert args.kd_temperature > 0, 'kd_temperature must be positive'
