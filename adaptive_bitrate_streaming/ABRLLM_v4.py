"""
ABRLLM v4：v2 架构 + Merina 风格带宽 β-VAE（latent 注入 state_emb，不替换 StateEncoder）。

``--bw-vae-fusion residual``：Linear(z)→hidden 后残差相加；``concat``：与 state_emb 拼接再融合。
"""
import torch
...
class ABRLLM(nn.Module):
    def __init__(self, args):
        ...
        # Merina 风格带宽 β-VAE → latent 注入 state_emb（v4 固定启用）
        self.bw_vae_kl_weight = float(getattr(args, 'bw_vae_kl_weight', 0.01))
        self.bw_vae_fusion = str(getattr(args, 'bw_vae_fusion', 'residual'))
        if self.bw_vae_fusion not in ('residual', 'concat'):
            raise ValueError(f'未知 bw_vae_fusion: {self.bw_vae_fusion!r}，可选 residual / concat')
        self._last_bw_vae_kl = None
        if not self.state_use_self_attention:
            raise ValueError('ABRLLM v4 需要 --state-use-self-attention')
        bw_latent_dim = int(getattr(args, 'bw_vae_latent_dim', 16))
        self.bandwidth_vae = BandwidthVAE(
            hist_len=6,
            latent_dim=bw_latent_dim,
            beta=float(getattr(args, 'bw_vae_beta', 0.4)),
        ).to(self.device)
        self.bw_latent_proj = nn.Linear(bw_latent_dim, self.state_attn_hidden_dim).to(self.device)
        if self.bw_vae_fusion == 'concat':
            self.bw_fusion_proj = nn.Linear(
                2 * self.state_attn_hidden_dim, self.state_attn_hidden_dim
            ).to(self.device)
        else:
            self.bw_fusion_proj = None
        ...
        modules_list.extend([self.bandwidth_vae, self.bw_latent_proj])
        if self.bw_fusion_proj is not None:
            modules_list.append(self.bw_fusion_proj)
    ...
    def _fuse_bandwidth_latent(self, states: torch.Tensor, state_emb: torch.Tensor) -> torch.Tensor:
        """StateEncoder 输出上融合带宽 VAE latent（残差或拼接）。"""
        z, kl = self.bandwidth_vae(states, sample=self.training)
        self._last_bw_vae_kl = kl
        bw_feat = self.bw_latent_proj(z)
        if self.bw_vae_fusion == 'concat':
            return self.bw_fusion_proj(torch.cat([state_emb, bw_feat], dim=-1))
        return state_emb + bw_feat

    def pop_bw_vae_kl_loss(self):
        """Trainer 取走上一步 forward 的 β-KLD（单次 backward 后调用）。"""
        kl = self._last_bw_vae_kl
        self._last_bw_vae_kl = None
        return kl
    ...
    def forward(self, states, actions, returns, timesteps, attention_mask=None):
        ...
        if self.state_use_self_attention:
            ...
            state_emb = self._fuse_bandwidth_latent(states, state_emb)
    ...
    def sample(self, state, target_return, timestep, **kwargs):
        ...
        if self.state_use_self_attention:
            ...
            state_emb = self._fuse_bandwidth_latent(state, state_emb)
